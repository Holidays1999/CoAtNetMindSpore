...
def parse_arguments():
    ...
    parser.add_argument('--data_url', default="/cache/data", help='location of data.')
    ...
    parser.add_argument('--train_url', default="/cache/output", help='location of training outputs.')
    parser.add_argument("--run_modelarts", type=ast.literal_eval, default=False, help="whether run on modelarts")
    parser.add_argument("--run_openai", type=ast.literal_eval, default=False, help="whether run on openai(V2)")
    parser.add_argument("--eval_every_epoch", type=ast.literal_eval, default=False, help="whether eval every epoch)")
...
def get_config():
    ...
    # make sure code either run on modelarts or run on openai
    assert args.run_modelarts == False or args.run_openai == False
    if args.run_modelarts or args.run_openai:
        try:
            config = args.config
            ...
            if (not args.config.startswith("obs:/")) and args.run_modelarts:
                config = "obs:/" + args.config
                with mox.file.File(config, 'r') as f:
                    yaml_txt = f.read()
            if (not args.config.startswith("s3:/")) and args.run_openai:
                config = "s3:/" + args.config
                with mox.file.File(config, 'r') as f:
                    yaml_txt = f.read()
    else:
        config = args.config
